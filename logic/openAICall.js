...
const getAdmissionDecision = require(appRoot +
  "/logic/getAdmissionDecision.js");
const checkIfUserExist = require(appRoot + "/logic/checkUserExist.js");
const getAssistantResponseFromIPGEData = require(appRoot +
  "/logic/getAssistantResponseFromIPGEData.js");
const getStudentId = require(appRoot + "/logic/getStudentId.js");
const getTools = require(appRoot + "/helpers/tools.json");
const { jwtDecode } = require("jwt-decode");
const client = openaiclient();

const conditionalCall = (history, user) => {
  return new Promise(async (resolve, reject) => {
    try {
        ...
        if (responseMessage.tool_calls) {
          for (const toolCall of responseMessage.tool_calls) {
            ...
            if (functionName === "getStudentInfo") {
              ...
              functionResponse = await checkIfUserExist(
                functionArgs.studentId,
                user
              );
            } else if (functionName === "getAdmissionDecision") {
              functionResponse = await getAdmissionDecision(
                functionArgs.studentId,
                user
              );
            }
          ...
          let functionResponse = await getAssistantResponseFromIPGEData(
            history
          );
          history.push({
            role: "assistant",
      ...
      resolve(finalResponse.choices[0].message.content);
    } catch (err) {
      if (err.includes(429)) {
        const match = err.match(/(\d+)\s+seconds/);
        if (match) {
        }
        resolve(`Bot limit reached, please try again in ${match[1]} seconds!!`);
      } else resolve(err);
    }
  });
};
